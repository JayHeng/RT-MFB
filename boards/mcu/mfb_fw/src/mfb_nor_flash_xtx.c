...
const uint32_t s_customLUT_XTX_Quad[CUSTOM_LUT_LENGTH] = {
    ...
    [MIXSPI_LUT_SUB_SEQ_LEN * NOR_CMD_LUT_SEQ_IDX_READ + 1] =
        MIXSPI_LUT_SEQ(kMIXSPI_Command_MODE8_SDR, kMIXSPI_4PAD, 0x00, kMIXSPI_Command_DUMMY_SDR, kMIXSPI_4PAD, XTX_QUAD_FLASH_DUMMY_CYCLES - 2),
...
void mfb_flash_set_param_for_xtx(jedec_id_t *jedecID)
{
    mfb_printf(" -- XTX Serial Flash.\r\n");
    ...
    if (!g_flashPropertyInfo.flashIsOctal)
    {
        ...
        g_flashPropertyInfo.flashBusyStatusPol        = XTX_FLASH_BUSY_STATUS_POL;
        g_flashPropertyInfo.flashBusyStatusOffset     = XTX_FLASH_BUSY_STATUS_OFFSET;
...
void mfb_flash_show_registers_for_xtx(bool isOctalFlash)
{
