...
const uint32_t s_customLUT_PUYA_Quad[CUSTOM_LUT_LENGTH] = {
    ...
    [MIXSPI_LUT_SUB_SEQ_LEN * NOR_CMD_LUT_SEQ_IDX_READ + 1] =
        MIXSPI_LUT_SEQ(kMIXSPI_Command_MODE8_SDR, kMIXSPI_4PAD, 0x00, kMIXSPI_Command_DUMMY_SDR, kMIXSPI_4PAD, PUYA_QUAD_FLASH_DUMMY_CYCLES - 2),
    ...
    // opcode 0x05/0x35 to read Status Registers (0/1)
    ...
    /* Enable Quad mode */
    // QE bit in 16bit Status Register [1]
    // opcode 0x01/0x31 to write Status Registers (0/1)
...
void mfb_flash_set_param_for_puya(jedec_id_t *jedecID)
{
    mfb_printf(" -- Puya Serial Flash.\r\n");
    ...
    if (!g_flashPropertyInfo.flashIsOctal)
    {
        ...
        g_flashPropertyInfo.flashBusyStatusPol        = PUYA_FLASH_BUSY_STATUS_POL;
        g_flashPropertyInfo.flashBusyStatusOffset     = PUYA_FLASH_BUSY_STATUS_OFFSET;
        g_flashPropertyInfo.flashQuadEnableCfg        = PUYA_FLASH_QUAD_ENABLE;
        g_flashPropertyInfo.flashQuadEnableBytes      = 1;
        g_flashPropertyInfo.mixspiCustomLUTVendor     = s_customLUT_PUYA_Quad;
    }
#endif
}

void mfb_flash_show_registers_for_puya(bool isOctalFlash)
{
    ...
    if (!isOctalFlash)
    {
        ...
        mixspi_nor_read_register(EXAMPLE_MIXSPI, &regAccess);
        mfb_printf("MFB: Flash Configure Register: 0x%x\r\n", regAccess.regValue.B.reg1);
    }
#endif
}
