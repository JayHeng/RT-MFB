...
uint8_t s_xmcFlashTypeIDs[] = {0x40};
...
const uint32_t s_customLUT_XMC_Quad[CUSTOM_LUT_LENGTH] = {
    ...
    [MIXSPI_LUT_SUB_SEQ_LEN * NOR_CMD_LUT_SEQ_IDX_READ + 1] =
        MIXSPI_LUT_SEQ(kMIXSPI_Command_MODE8_SDR, kMIXSPI_4PAD, 0x00, kMIXSPI_Command_DUMMY_SDR, kMIXSPI_4PAD, XMC_QUAD_FLASH_DUMMY_CYCLES - 2),
...
void mfb_flash_set_param_for_xmc(jedec_id_t *jedecID)
{
    mfb_printf(" -- XMC Serial Flash.\r\n");
    mfb_printf("MFB: Flash Memory Type ID: 0x%x", jedecID->memoryTypeID);
    switch (jedecID->memoryTypeID)
    {
        /////////////////////////QuadSPI////////////////////////
        case 0x40:
            mfb_printf(" -- XM25QH QuadSPI 3.3V Series.\r\n");
    ...
    if (!g_flashPropertyInfo.flashIsOctal)
    {
        ...
        g_flashPropertyInfo.flashBusyStatusPol        = XMC_FLASH_BUSY_STATUS_POL;
        g_flashPropertyInfo.flashBusyStatusOffset     = XMC_FLASH_BUSY_STATUS_OFFSET;
        g_flashPropertyInfo.flashQuadEnableCfg        = XMC_FLASH_QUAD_ENABLE;
        g_flashPropertyInfo.flashQuadEnableBytes      = 1;
        g_flashPropertyInfo.mixspiCustomLUTVendor     = s_customLUT_XMC_Quad;
    }
#endif
}

void mfb_flash_show_registers_for_xmc(bool isOctalFlash)
{
