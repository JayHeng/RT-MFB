...
const uint32_t s_customLUT_FMSH_Quad[CUSTOM_LUT_LENGTH] = {
    ...
    [MIXSPI_LUT_SUB_SEQ_LEN * NOR_CMD_LUT_SEQ_IDX_READ + 1] =
        MIXSPI_LUT_SEQ(kMIXSPI_Command_MODE8_SDR, kMIXSPI_4PAD, 0x00, kMIXSPI_Command_DUMMY_SDR, kMIXSPI_4PAD, FMSH_QUAD_FLASH_DUMMY_CYCLES - 2),
    ...
    // opcode 0x05 to read Status Register-1, opcode 0x35 to read Status Register-2
    ...
    /* Enable Quad mode */
    // QE bit in Status Register-2[1] (S9), there are two Status Registers (1/2)
...
void mfb_flash_set_param_for_fmsh(jedec_id_t *jedecID)
{
    mfb_printf(" -- FMSH Serial Flash.\r\n");
    mfb_printf("MFB: Flash Memory Type ID: 0x%x", jedecID->memoryTypeID);
    switch (jedecID->memoryTypeID)
    {
        /////////////////////////QuadSPI////////////////////////
        case 0x60:
            mfb_printf(" -- FM25LQ QuadSPI 1.8V Series.\r\n");
            break;
        default:
    ...
    if (!g_flashPropertyInfo.flashIsOctal)
    {
        ...
        g_flashPropertyInfo.flashBusyStatusPol        = FMSH_FLASH_BUSY_STATUS_POL;
        g_flashPropertyInfo.flashBusyStatusOffset     = FMSH_FLASH_BUSY_STATUS_OFFSET;
        g_flashPropertyInfo.flashQuadEnableCfg        = FMSH_FLASH_QUAD_ENABLE;
        g_flashPropertyInfo.flashQuadEnableBytes      = 1;
        g_flashPropertyInfo.mixspiCustomLUTVendor     = s_customLUT_FMSH_Quad;
    }
#endif
}

void mfb_flash_show_registers_for_fmsh(bool isOctalFlash)
{
