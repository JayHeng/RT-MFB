...
#include "mfb_nor_flash_giantec.h"
...
const uint32_t s_customLUT_GIANTEC_Quad[CUSTOM_LUT_LENGTH] = {
    ...
    [MIXSPI_LUT_SUB_SEQ_LEN * NOR_CMD_LUT_SEQ_IDX_READ + 1] =
        MIXSPI_LUT_SEQ(kMIXSPI_Command_MODE8_SDR, kMIXSPI_4PAD, 0x00, kMIXSPI_Command_DUMMY_SDR, kMIXSPI_4PAD, GIANTEC_QUAD_FLASH_DUMMY_CYCLES - 2),
    ...
    // opcode 0x15 to read Status Register-3
...
void mfb_flash_set_param_for_giantec(jedec_id_t *jedecID)
{
    mfb_printf(" -- Giantec Serial Flash.\r\n");
    mfb_printf("MFB: Flash Memory Type ID: 0x%x", jedecID->memoryTypeID);
    switch (jedecID->memoryTypeID)
    {
        /////////////////////////QuadSPI////////////////////////
        case 0x60:
            mfb_printf(" -- GT25Q QuadSPI 3.3V Series.\r\n");
    ...
    if (!g_flashPropertyInfo.flashIsOctal)
    {
        ...
        g_flashPropertyInfo.flashBusyStatusPol        = GIANTEC_FLASH_BUSY_STATUS_POL;
        g_flashPropertyInfo.flashBusyStatusOffset     = GIANTEC_FLASH_BUSY_STATUS_OFFSET;
        g_flashPropertyInfo.flashQuadEnableCfg        = GIANTEC_FLASH_QUAD_ENABLE;
        g_flashPropertyInfo.flashQuadEnableBytes      = 1;
        g_flashPropertyInfo.mixspiCustomLUTVendor     = s_customLUT_GIANTEC_Quad;
    }
#endif
}

void mfb_flash_show_registers_for_giantec(bool isOctalFlash)
{
