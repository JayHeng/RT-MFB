...
const uint32_t s_customLUT_WINBOND_Quad[CUSTOM_LUT_LENGTH] = {
    ...
    [4 * NOR_CMD_LUT_SEQ_IDX_READ + 1] =
        MIXSPI_LUT_SEQ(kFLEXSPI_Command_MODE8_SDR, kMIXSPI_4PAD, 0xF0, kFLEXSPI_Command_DUMMY_SDR, kMIXSPI_4PAD, WINBOND_QUAD_FLASH_DUMMY_CYCLES - 2),
...
const uint32_t s_customLUT_WINBOND_Octal[CUSTOM_LUT_LENGTH] = {
    ...
    [4 * NOR_CMD_LUT_SEQ_IDX_READ + 1] = 
        MIXSPI_LUT_SEQ(kFLEXSPI_Command_DUMMY_SDR, kMIXSPI_8PAD, 0x08, kFLEXSPI_Command_READ_SDR,  kMIXSPI_8PAD, 0x04),
#endif
