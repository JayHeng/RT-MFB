...
{
    return (uint8_t)((base->TRDC_HWCFG1 & TRDC_TRDC_HWCFG1_DID_MASK) >> TRDC_TRDC_HWCFG1_DID_SHIFT);
}
...
{
    base->TRDC_CR |= TRDC_TRDC_CR_GVLDM_MASK;
}
...
#endif
{
    ...
    assert((uint32_t)master < ((base->TRDC_HWCFG0 & TRDC_TRDC_HWCFG0_NMSTR_MASK) >> TRDC_TRDC_HWCFG0_NMSTR_SHIFT));
#if defined(FSL_FEATURE_TRDC_PROCESSOR_MASTER_COUNT) && (FSL_FEATURE_TRDC_PROCESSOR_MASTER_COUNT > 1)
    if (0U == (base->DACFG[master] & TRDC_DACFG_NCM_MASK))
    {
        base->MDA_DFMT0[master].MDA_W_DFMT0[regNum] |= TRDC_MDA_W_DFMT0_LK1_MASK;
    }
    else
    {
        base->MDA_DFMT1[master].MDA_W_DFMT1[0] |= TRDC_MDA_W_DFMT1_LK1_MASK;
    }
#else
    /* Non-processor master */
    if ((uint8_t)master != 0U)
    {
        base->MDA_W0_DFMT1[(uint8_t)(master - 1U)].MDA_W0_x_DFMT1 |= TRDC_MDA_W0_x_DFMT1_LK1_MASK;
    }
    /* Processor master */
    else
    {
        base->MDA_W0_0_DFMT0 |= TRDC_MDA_W0_0_DFMT0_LK1_MASK;
    }
#endif
}
...
#endif
{
    ...
    if (valid)
    {
        ...
        {
            base->MDA_DFMT1[master].MDA_W_DFMT1[0] |= TRDC_MDA_W_DFMT1_VLD_MASK;
        }
    }
    else
    {
    ...
    if ((uint8_t)master != 0U)
    {
        if (valid)
        {
            base->MDA_W0_DFMT1[(uint8_t)(master - 1U)].MDA_W0_x_DFMT1 |= TRDC_MDA_W0_x_DFMT1_VLD_MASK;
        }
        else
        {
            base->MDA_W0_DFMT1[(uint8_t)(master - 1U)].MDA_W0_x_DFMT1 &= ~TRDC_MDA_W0_x_DFMT1_VLD_MASK;
...
static inline void TRDC_EnableFlashLogicalWindow(TRDC_Type *base, bool enable)
{
    if (enable)
    {
        base->TRDC_FLW_CTL |= TRDC_TRDC_FLW_CTL_V_MASK;
    }
    else
    {
        base->TRDC_FLW_CTL &= ~TRDC_TRDC_FLW_CTL_V_MASK;
...
{
    base->TRDC_FLW_CTL |= TRDC_TRDC_FLW_CTL_LK_MASK;
}
...
{
    base->TRDC_CR |= TRDC_TRDC_CR_GVLDR_MASK;
}
...
{
    base->TRDC_CR |= TRDC_TRDC_CR_GVLDB_MASK;
}
